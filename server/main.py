    ...
    try:
        with open(STATE_FILE, "r") as f:
            ...
            if not isinstance(state, dict):
                state = {"session_active": False, "session_end_time": 0, "last_run": 0, "last_expiry_run": 0}
...
def main():
    ...
    while True:
        ...
        if state.get("session_active"):
            ...
            # Check if it's time to run product check (every 10 mins)
            last_product_run = state.get("last_run", 0)
            if (now - last_product_run) > PRODUCT_CHECK_INTERVAL:
                check_products(wcapi, finder, categorizer)
                state["last_run"] = now
                save_state(state)
            
            # Check if it's time to run expiry alerts (every hour)
            last_expiry_run = state.get("last_expiry_run", 0)
            if (now - last_expiry_run) > EXPIRY_CHECK_INTERVAL:
